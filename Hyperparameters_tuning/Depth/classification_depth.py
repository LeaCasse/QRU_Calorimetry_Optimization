...
# Training loop for different depths
results = []
log_file = f"train_qml_classification_depth_{timestamp}.log"
with open(log_file, "w") as f:
    for depth in depths:
        ...
        plt.savefig(f"Distribution_of_parameters_values_depth_{depth}_{timestamp}.png", dpi=300)
        plt.show()
        ...
        plt.savefig(f"Loss_Accuracy_depth_{depth}_{timestamp}.png", dpi=300)
        plt.show()

# Analyze results
depths = [result['depth'] for result in results]
trainabilities = [result['trainability'] for result in results]
times = [result['time_process'] for result in results]
final_losses = [result['final_loss_ma'] for result in results]
final_train_accuracies = [result['final_train_acc_ma'] for result in results]
final_test_accuracies = [result['final_test_acc_ma'] for result in results]
...
plt.savefig(f"Trainability_vs_Depth_{timestamp}.png", dpi=300)
plt.show()
...
plt.savefig(f"Execution_Time_vs_Depth_{timestamp}.png", dpi=300)
plt.show()
...
plt.savefig(f"Final_Loss_vs_Depth_{timestamp}.png", dpi=300)
plt.show()
...
plt.savefig(f"Final_Train_Accuracy_vs_Depth_{timestamp}.png", dpi=300)
plt.show()
...
plt.savefig(f"Final_Test_Accuracy_vs_Depth_{timestamp}.png", dpi=300)
plt.show()
