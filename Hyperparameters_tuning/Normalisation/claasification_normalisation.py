...
plt.annotate(f'Final mean loss: {final_loss_ma_pi:.6f}', xy=(epochs_pi[-1], loss_hist_ma_pi[-1]), 
             xytext=(epochs_pi[0]+4, final_loss_ma_pi+0.2),
...
plt.annotate(f'Final mean train acc: {final_train_acc_ma_pi:.6f}', xy=(epochs_pi[-1], acc_hist_ma_pi[-1]), 
             xytext=(epochs_pi[0]+4, final_train_acc_ma_pi -0.08),
             arrowprops=dict(facecolor='black', width=0.5, headwidth=5, shrink=0.05))
plt.annotate(f'Final mean test acc: {final_test_acc_ma_pi:.6f}', xy=(epochs_pi[-1], test_acc_hist_ma_pi[-1]), 
             xytext=(epochs_pi[0]+4, final_test_acc_ma_pi - 0.1),
             arrowprops=dict(facecolor='black', width=0.5, headwidth=5, shrink=0.05))
...
plt.ylabel('Loss')
plt.legend(loc='upper right')
plt.annotate(f'Final mean loss: {final_loss_ma_2pi:.6f}', xy=(epochs_2pi[-1], loss_hist_ma_2pi[-1]), 
             xytext=(epochs_2pi[0]+4, final_loss_ma_2pi+0.2),
             arrowprops=dict(facecolor='black', width=0.5, headwidth=5, shrink=0.05))
...
plt.ylabel('Accuracy')
plt.legend(loc='lower right')
plt.annotate(f'Final mean train acc: {final_train_acc_ma_2pi:.6f}', xy=(epochs_2pi[-1], acc_hist_ma_2pi[-1]), 
             xytext=(epochs_2pi[0]+4, final_train_acc_ma_2pi -0.08),
             arrowprops=dict(facecolor='black', width=0.5, headwidth=5, shrink=0.05))
plt.annotate(f'Final mean test acc: {final_test_acc_ma_2pi:.6f}', xy=(epochs_2pi[-1], test_acc_hist_ma_2pi[-1]), 
             xytext=(epochs_2pi[0]+4, final_test_acc_ma_2pi - 0.1),
             arrowprops=dict(facecolor='black', width=0.5, headwidth=5, shrink=0.05))

plt.savefig(os.path.join(output_dir, "Loss_Accuracy_Comparison.png"), dpi=300)
plt.show()
...
# Trainability
axs[0, 0].bar(['-pi to pi', '0 to 2pi'], [trainability_pi, trainability_2pi], color=['blue', 'orange'])
axs[0, 0].set_title('Trainability')

# Execution time
axs[0, 1].bar(['-pi to pi', '0 to 2pi'], [time_process_pi, time_process_2pi], color=['blue', 'orange'])
axs[0, 1].set_title('Execution Time (seconds)')

# Final loss
axs[0, 2].bar(['-pi to pi', '0 to 2pi'], [final_loss_ma_pi, final_loss_ma_2pi], color=['blue', 'orange'])
axs[0, 2].set_title('Final Mean Loss')

# Final train accuracy
axs[1, 0].bar(['-pi to pi', '0 to 2pi'], [final_train_acc_ma_pi, final_train_acc_ma_2pi], color=['blue', 'orange'])
axs[1, 0].set_title('Final Mean Train Accuracy')

# Final test accuracy
axs[1, 1].bar(['-pi to pi', '0 to 2pi'], [final_test_acc_ma_pi, final_test_acc_ma_2pi], color=['blue', 'orange'])
